#!/usr/bin/env bash
# Generate a per-tag changelog: the commits (and full diff link) between the
# previous vX.X.X tag and the tag being released.
set -euo pipefail
...

{
  ...
  echo "## Downloads"
  echo
  echo "GitHub displays a SHA-256 digest for each release asset. Every binary is"
  echo "covered by a GitHub build-provenance attestation, and the Windows artifacts"
  echo "were scanned with Microsoft Defender before publication."
} >"$OUTPUT_PATH"
